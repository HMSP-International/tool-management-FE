...
const SpaceInvited: React.FC<IProps> = () => {
	...
	return (
		<Menu mode='inline' inlineCollapsed={false} key={'menu2'}>
			<SubMenu key='space' icon={<AppstoreOutlined />} title={<TitleSpace title={'Invited Space'} />}>
				{collaboratorRedux.collaborators.map(collaborator => {
					...
					return (
						<SubMenu key={space._id} icon={<AppstoreOutlined />} title={<TitleSpace title={space.name} />}>
							{keys.map(key =>
								projectRedux.projectsFromCollaborator[key].map(project => (
									<Menu.Item key={project._id} icon={<AppstoreOutlined />}>
										<Link to={`/${mainRouterPage.taskDoing.index}/${project._id}`}>
											{project.name}
										</Link>
									</Menu.Item>
								)),
